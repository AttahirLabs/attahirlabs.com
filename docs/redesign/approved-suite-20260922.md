...
## Animated business elevator prototype
The homepage glass hero now connects four miniature businesses to relevant suite offerings: café → web design, warehouse → StockClearance, grocer → ShelfLife, and homewares → TariffShield. The right-hand rooms travel vertically on a shared timeline; the foreground glass interface rotates in sync. Each business holds for 6.5 seconds before a 1.35-second transition. Visitors can select a business or pause all scene movement.

The miniature people use deterministic articulated walking, seated sipping, preparation, packing, restocking, and customer interaction cycles. These are explicitly illustrative businesses and concept interfaces, not customer case studies or live product data. The renderer uses locally hosted Three.js 0.186.0 under its MIT license and custom geometry; it makes no external asset requests. Each scene contains 89–242 meshes, sharing geometries and materials. The render loop is capped at 30fps, stops off-screen or when the tab is hidden, and honors reduced motion. Keyboard focus anywhere in the scene pauses autoplay, and pausing during a transition settles to a fully readable panel. Explicit Play restarts the scene.

The homepage retains a rendered still poster if scripting or WebGL is unavailable. Reduced-motion users see a still 3D scene and can select scenes without animation. The poster was captured from this implementation in Chrome and optimized to WebP, not generated from a separate visual concept. Styles and scripts are loaded only on the homepage. Renderer and scene resources are released on navigation; BFCache restores retain the paused state.

Validation: timeline tests cover dwell, pause, wraparound, manual selection and background-clock bounds. `tests/commerce-browser.cjs` compares moving and paused canvas frames and checks scene/panel pairing, keyboard pause and restart, mid-transition pause, tablet/mobile control placement, reduced motion, failed-renderer fallback, and no-JavaScript fallback. The existing site browser suite and deterministic tests remain required.
