...
const install=(p,where='app_page_hero')=>p.slug==='stockclearance'?`${p.listing}?utm_source=attahirlabs&utm_medium=website&utm_campaign=stockclearance&utm_content=${where}`:p.listing;
...
function main(p){
 ...
 const hero=`<section class="page-hero"><div class="container hero-grid"><div><div class="eyebrow">Available on the Shopify App Store</div>${heading}<h1>${esc(p.headline)}</h1><p class="lede">${esc(p.description)}</p><div class="button-row">${cta(p,'app_page_hero')}<a class="btn btn-secondary" href="#pricing">Compare plans</a></div><p class="plan-summary">${esc(p.priceSummary)}<br><span>${esc(p.trialSummary)}</span></p></div><figure class="product-shot"><a href="${shot.src}" target="_blank" rel="noopener" aria-label="View ${esc(p.name)} screenshot at full size"><img class="product-screen" src="${shot.src}" alt="${esc(shot.alt)}" width="1280" height="720" fetchpriority="high"></a><figcaption>From the current App Store listing. Example data shown.</figcaption></figure></div></section>`;
 ...
 return `<main class="product-page" id="main">\n${[hero,links,features,screenshots,setup,pricing,faq,footer].join('\n')}\n</main>`;
}
...
for(const p of catalog.products){
 ...
 html=html.replace('</head>',`<link rel="stylesheet" href="/apps/product-pages.css?v=20260913">\n${schemas(p)}\n</head>`);
 write(relative,html);
}
const cards=catalog.products.map(p=>`<article class="card app-card"><div class="app-heading"><img class="app-icon" src="${p.icon}" alt="${p.slug==='shelflife'?'ShelfLife app icon':esc(p.iconAlt)}" width="48" height="48"><div><span class="tag tag-live">Shopify App Store</span><h3>${p.name}</h3></div></div><p>${esc(p.summary)}</p>${list(p.homeFeatures)}<p class="home-plan-summary">${esc(p.priceSummary)}<br><span>${esc(p.trialSummary)}</span></p><div class="button-row"><a class="btn btn-dark" href="${esc(install(p,'homepage_public_apps'))}" rel="noopener" target="_blank">Install ${p.name}</a><a class="btn btn-light" href="/apps/${p.slug}/">Features &amp; pricing</a></div></article>`).join('\n');
